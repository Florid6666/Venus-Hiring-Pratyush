    ...
    handlers: {
      GET: async () => {
        try {
          ...
          console.error("[PostgreSQL GET /api/blogs Warning]:", errorMsg);
          // Return static fallback blogs so site rendering never breaks
          const { INITIAL_BLOGS, INITIAL_CATEGORIES } = await import("@/lib/blog-store");
          return new Response(
            JSON.stringify({
              success: true,
              isFallback: true,
